...
public class WebScraperHelper {
     ...
     * @return : returns a set of all movies with the details fetched using the links search tag supplied in constructor.
     */
    ...
    /**
     * Extracts movie details from meta tag using the detailsSearchTag supplied in constructor.
     * @param url
     * @return
     * @throws IOException
     */
    private Map<String, String> getLinkDetails(String url) throws IOException{
        ...
        detailsSearchTag.forEach(tag->{
            tagDetails.put(tag, doc.select(tag).text());
        });
        return tagDetails;
    }
    /**
